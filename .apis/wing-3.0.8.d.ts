/*
	This is the Type Definition file for EgretWing version 3.0.8
*/
...
declare namespace wing {
	...
	export class Store {
		...
		validateValue(name: string, value?: any): boolean;

		setValidator(validator: (name: string, value: any) => Thenable<string>): void;
	...
	export interface SimpleTemplateFileContext extends TemplateFileContext {
	...
	export interface SimpleTemplateFileProvider {
		 ...
		 */
		provideSimpleTemplateFile(context: SimpleTemplateFileContext, token: CancellationToken): Thenable<string>;
	}

	export interface TemplateFileProvider {
		 ...
		 */
		provideTemplateFile(context: TemplateFileContext, token: CancellationToken): Thenable<ITemplateFile>;
	}
	...
	export interface TemplateProjectProvider {
		label: string;
		icon?: string | { light: string, dark: string };
		detail?: string;
		provideTemplateProject(context: TemplateProjectContext, token: CancellationToken): Thenable<ITemplateProject>;
	}
	...
	/**
	 * Namespace for create template.
	 */
	export namespace template {
		/**
		 * Register a template file provider.
		 *
		 * @param id The action id.
		 * @param provider A provider.
		 */
		export function registerTemplateFileProvider(id: string, provider: TemplateFileProvider | SimpleTemplateFileProvider): Disposable;

		/**
		 * Register a template project provider.
		 */
		export function registerTemplateProjectProvider(id: string, provider: TemplateProjectProvider): Disposable;
	}
