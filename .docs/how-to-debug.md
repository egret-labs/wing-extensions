...
以上命令指定了一个特定插件的位置，并开启5940端口，等待调试器连接。

## 启动node-inspector

接下来开启一个命令行窗口，启动node-inspector

	$ node-inspector --debug-port=5940

按照提示，打开 `chrome浏览器` 访问网址

	http://127.0.0.1:8080/?ws=127.0.0.1:8080&port=5940

切换到 `Sources` 标签页即可看到相关的源代码，并可以在任意位置设置断点。

![chrome-debug](/.docs/img/chrome-debug.png)
